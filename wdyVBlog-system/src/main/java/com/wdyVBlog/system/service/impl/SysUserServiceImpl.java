...
import com.wdyVBlog.common.annotation.DataScope;
...
import com.wdyVBlog.common.utils.bean.BeanValidators;
import com.wdyVBlog.common.utils.spring.SpringUtils;
import com.wdyVBlog.system.domain.SysPost;
import com.wdyVBlog.system.domain.SysUserPost;
import com.wdyVBlog.system.domain.SysUserRole;
import com.wdyVBlog.system.mapper.SysPostMapper;
import com.wdyVBlog.system.mapper.SysRoleMapper;
import com.wdyVBlog.system.mapper.SysUserMapper;
import com.wdyVBlog.system.mapper.SysUserPostMapper;
import com.wdyVBlog.system.mapper.SysUserRoleMapper;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public PageResult<SysUser> selectUserList(SysUser user) {
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Page<SysUser> sysUserPage = new Page<>(pageDomain.getPageNum(), pageDomain.getPageSize());
        Page<SysUser> page = this.baseMapper.selectUserPage(sysUserPage, user);
//        return this.baseMapper.selectUserList(user);
        return new PageResult<SysUser>(page);
    }
    ...
    @Override
    @DataScope(deptAlias = "d", userAlias = "u")
    public PageResult<SysUser> selectAllocatedList(SysUser user) {
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Page<SysUser> page = new Page<>(pageDomain.getPageNum(), pageDomain.getPageSize());
        Page<SysUser> sysUserIPage = this.baseMapper.selectAllocatedPage(page, user);
        return new PageResult<SysUser>(sysUserIPage);
    }
    ...
    @Override
    @DataScope(deptAlias = "d", userAlias = "u")
    public PageResult<SysUser> selectUnallocatedList(SysUser user) {
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Page<SysUser> page = new Page<>(pageDomain.getPageNum(), pageDomain.getPageSize());
        Page<SysUser> sysUserIPage = this.baseMapper.selectUnallocatedPage(page, user);
        return new PageResult<SysUser>(sysUserIPage);
//        return this.baseMapper.selectUnallocatedList(user);
    ...
    public String checkUserNameUnique(SysUser user) {
        ...
        SysUser info = this.baseMapper.checkUserNameUnique(user.getUserName());
    ...
    public String checkPhoneUnique(SysUser user) {
        Long userId = StringUtils.isNull(user.getUserId()) ? -1L : user.getUserId();
        SysUser info = this.baseMapper.checkPhoneUnique(user.getPhonenumber());
        if (StringUtils.isNotNull(info) && info.getUserId().longValue() != userId.longValue()) {
            return UserConstants.NOT_UNIQUE;
        }
        return UserConstants.UNIQUE;
    ...
    public void checkUserDataScope(Long userId) {
        if (!SysUser.isAdmin(SecurityUtils.getUserId())) {
            ...
            PageResult<SysUser> users = SpringUtils.getAopProxy(this).selectUserList(user);
